...
- ### CUnit
    ...
    ```JavaScript
    const unit = CUnit.cast(v)
        函数将函数参数或返回值强制转换为 CUnit 对象
    const unit = CUnit(EPD)
        函数从结构偏移 EPD 值创建 CUnit 对象
    const unit = CUnit(EPD, ptr=ptr)
        函数从结构偏移 EPD 值和 ptr 值创建 CUnit 对象
    const unit = CUnit.from_read(EPD)
        函数从存储 EPD 地址的值读取并创建 CUnit 对象。 如果地址为空则 unit 为 0
    const unit = CUnit.from_ptr(ptr)
        函数从 ptr 值计算 EPD 并创建 CUnit 类型。在调用位置缓存 ptr 值，避免重复计算 EPD
    const unit = CUnit 实例成员中的 CUnit
    ```

    ```JavaScript
    // 示例：增加资源收集超过 256
    const bonusMineral = PVariable(list(492, 0, 0, 0, 0, 0, 0, 0));  // 红色为 492 + 8 = 收集最多 500 个水晶矿
    const bonusGas = PVariable(list(992, 0, 0, 0, 0, 0, 0, 0));  // 红色为 992 + 8 = 最多收集 1000 个气矿
    function loopUnit() {
...
- ### UnitGroup
    ...
    UnitGroup 是一个经过 CPTrick 优化的单位实例容器

    ```JavaScript
    object GUnit {
        function remove(){}           // 把自身从所属的 UnitGroup 中删除
        const dying : EUDGUnitIter;   // 迭代死掉的单位并把死掉的单位从所属的 UnitGroup 中删除
    }
