...
- ## 编译期字符串（py_str）
    ...
    编译期字符串是指在编译期产生、最终不会插入地图的字符串，它包含所有字面量字符串（literal string）以及编译期字符串常量。  
    因为 epScript 的编译期由 Python 实现，所以将它称为 py_str。编译期字符串不能直接用于地图运行时。  
    ...
    ```JavaScript
    SetDeaths(P1, Add, 10, "Terran Marine"); // SetDeaths 动作对应参数的类型是 TrgUnit，所以它会在编译期被替换成 TrgUnit 表中对应的整数编号传入
    ```

    上述代码中的 "Terran Marine" 这个字符串不会插入到地图中。

    "Terran Marine" 只是用于在编译期查询该单位类型的编号（机枪兵的编号为 0）。
    ...
    在 epScript 中，也可以使用 py_str 声明一个编译期字符串常量。
    ...
    在编译期，可以通过编号索引宏获取编译期字符串在不同常量表中的编号。
    ...
    可以使用 EncodeString 宏将编译期字符串插入地图，使其成为地图字符串（TrgString），并返回它在地图字符串表中的编号。
...
- ## 地图字符串（TrgString）
    ...
    当我们在地图编辑器中使用任何自定义字符串时，地图编辑器会将这个字符串插入地图文件中的地图字符串表（Map String Table），并赋予一个编号。  
    例如，我们将地图标题（Map Title）设为 “一张好玩的星际争霸地图”，这个地图标题就是一个地图字符串（TrgString），它通常会被赋予编号 1。  
    触发器动作函数的字符串参数就是这个编号。
    ...
    在 epScript 代码中，如果函数的参数类型为 TrgString，而传入的参数是编译期字符串，则该字符串会先被插入地图字符串表中，然后将它的编号传入该参数位置。
    ...
    也可以构造一个很长的编译期空字符串，用 EncodeString 宏转换成地图字符串（TrgString），当作字符串缓冲区来充当字符串变量。
    ...
    当然，没有必要自己构建并维护地图字符串（TrgString）作为字符串缓冲区；使用 StringBuffer 对象可以更便捷地动态管理特定长度的地图字符串（TrgString）。
    ...
    更方便的是，epScript 提供了一系列 print 函数，使向屏幕输出文字时不需要自行维护 StringBuffer。

    epScript 中维护着一个容量为 1023 字节的全局 StringBuffer，用于实现 print 系列函数。也就是说，小于这个长度的字符串可以直接通过 print 系列函数输出。
...
- ## 字符串数据（Db）
    ...
    Db 字符串也是一种运行时自定义字符串。  
    但是它们不像地图字符串（TrgString）那样拥有编号，所以不能直接用于传统触发器动作。  
    通常需要用 Db 存储一些运行时组成字符串的片段，它们没有必要每一个都拥有编号。  
    可以将这些数据拷贝到 StringBuffer 或其他 TrgString，以传递给传统触发器动作。  
...
- ## TBL 字符串（stat_txt.tbl）
    ...
    TBL 字符串是指星际争霸 1 内部字符串表中的字符串，它包含单位名称、科技名称、技能名称等。  
    不能向 TBL 字符串表中添加字符串，可以从 [StatText](Constants-Reference/StatText.md) 找到所有 TBLKey 对应的编号。  
    TBLKey 对应的内存中的字符串并不等于 TBLKey 本身。  

    ```JavaScript
    println("{:s}", GetTBLAddr("Terran Siege Tank (Tank Mode)")); // 输出：Terran Siege Tank
    ```

    可以通过修改 TBL 表中的字符串来修改单位或技能名称。

    ```JavaScript
    dbstr_print(GetTBLAddr("Terran Marine"), "机枪兵\x00"); // 修改 tbl 会导致本地化失效，所有未修改的单位和技能名称都会变为英文，强烈不建议修改 tbl
    ```
